...
for (student in students) {
  # 組出完整路徑
  temp_path <- file.path(base_dir, student, "Midterm 1", "TEMP.csv")
...
p <- ggplot(data = all_data, aes(x = SampleIndex, y = Temperature, color = Student)) +
  geom_line(size = 0.7) +
  labs(
    title = "Midterm 1 Temperature Time Series",
    x = "Sample Index",
    y = "Temperature (°C)"
  ) +
  theme_minimal() +
...
# 7. 儲存圖檔
ggsave("midterm1_temperature_plot.png", plot = p, width = 15, height = 6, dpi = 300)
